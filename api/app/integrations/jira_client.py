...
class JiraClient:
    ...
    def create_issue(
        self, project_key, summary, description, issue_type="Task", custom_fields=None
    ):
        ...
        response = self.session.post(
            url, json={"fields": fields}, headers=self.headers, auth=self.auth
        )
        return self._handle_response(response)
    ...
    def update_issue(self, issue_key, updates):
        ...
        response = self.session.put(
            url, json={"fields": updates}, headers=self.headers, auth=self.auth
        )
        return self._handle_response(response)
    ...
    def add_comment(self, issue_key, comment):
        ...
        response = self.session.post(
            url, json={"body": comment}, headers=self.headers, auth=self.auth
        )
        return self._handle_response(response)
    ...
    def add_attachment(self, issue_key, file_path):
        ...
        with open(file_path, "rb") as f:
            files = {"file": f}
            response = self.session.post(
                url, headers=headers, files=files, auth=self.auth
            )
            return self._handle_response(response)
    ...
    def log_work(self, issue_key, time_spent, comment=None):
        ...
        payload = {"timeSpent": time_spent, "comment": comment or "Logged via IWAS"}
        response = self.session.post(
            url, json=payload, headers=self.headers, auth=self.auth
        )
        return self._handle_response(response)
...
if __name__ == "__main__":
    ...
    if not BASE_URL or not EMAIL or not API_TOKEN:
        print(
            "❗ ERROR: Missing one or more required environment variables (JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN)."
        )
        exit(1)
